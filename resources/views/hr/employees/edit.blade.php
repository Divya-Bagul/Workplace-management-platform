            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6 space-y-6">
                <form method="post" action="{{ route('employees.update', $employee) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="designation" name="designation" type="text" class="mt-1 block w-full" :value="old('designation', $employee->designation)" />
                        <x-input-error class="mt-2" :messages="$errors->get('designation')" />
                    </div>
                    <div>
                        <x-input-label for="joining_date" :value="__('Joining date')" />
                        <x-text-input id="joining_date" name="joining_date" type="date" class="mt-1 block w-full" :value="old('joining_date', optional($employee->joining_date)->format('Y-m-d'))" />
                    ...
                    <div>
                        ...
                        <select id="reporting_manager_id" name="reporting_manager_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            @foreach ($managers as $m)
                                @continue($m->id === $employee->id)
                                <option value="{{ $m->id }}" @selected(old('reporting_manager_id', $employee->reporting_manager_id) == $m->id)>{{ $m->name }} ({{ $m->employee_code }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="building_id" name="building_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach ($buildings as $b)
                                    <option value="{{ $b->id }}" @selected(old('building_id', $employee->building_id) == $b->id)>{{ $b->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="floor_id" name="floor_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <option value="">{{ __('— Select —') }}</option>
                                @foreach ($floors as $f)
                                    <option value="{{ $f->id }}" @selected(old('floor_id', $employee->floor_id) == $f->id)>{{ $f->building?->name }} · {{ $f->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select id="employment_status" name="employment_status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            @foreach (['active', 'inactive', 'on_leave'] as $st)
                                <option value="{{ $st }}" @selected(old('employment_status', $employee->employment_status) === $st)>{{ $st }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('employees.index') }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('Back') }}</a>
                    </div>
                </form>
            </div>
        </div>
